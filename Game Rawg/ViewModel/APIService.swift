...
fileprivate let apiUrlBase = "https://api.rawg.io/api"
...
class APIService : ServiceProtocol {
    func fetchSearchGame(search: String,completion: @escaping ([Game]?) -> Void) {
        ...
        getData(url: url) { (data) in
            ...
            game.results?.forEach({ (result) in
                games.append(Game(id: result.id ?? 0, judul: result.name ?? "Unknnown", gambar: result.backgroundImage ?? "Unavailable!", tanggalRilis: result.released ?? "Undefine", peringkat: Double(result.rating ?? 0)))
            })
    ...
    func fetchListGame(completion: @escaping ([Game]?) -> Void) {
        ...
        getData(url: url) { (data) in
            ...
            var games : [Game] = []
            game.results?.forEach({ (result) in
                games.append(Game(id: result.id ?? 0, judul: result.name ?? "", gambar: result.backgroundImage ?? "", tanggalRilis: result.released ?? "", peringkat: result.rating ?? 0.0))
            })
            DispatchQueue.main.async {
                completion(games)
            }
    ...
    func fetchDetailGame(id: String,completion: @escaping (GameDetail?) -> Void) {
        ...
        getData(url: url) { (data) in
            ...
            let detailGame = GameDetail(id: game.id ?? 0 , judul: game.name ?? "", deskripsi: game.descriptionRaw ?? "", gambar: game.backgroundImage ?? "", tanggalRilis: game.released ?? "", peringkat: game.rating ?? 0.0)
